// T302 — endgame triggered by six consecutive scoreless turns.
...
d('endgame: six-pass termination (US3)', () => {
  ...
  it('completes the game and clears active_slot after six consecutive passes', async () => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (!result.ok) throw new Error(`passTurn ${i} failed`);
      view = result.data;
    }
